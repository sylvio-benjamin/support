...
// Fonction pour obtenir la requête de comptage des messages non lus
        ...
        return "(SELECT COUNT(*) FROM conversation c 
                ...
                AND (
                    NOT EXISTS (
                        ...
                        AND ml.typeUtilisateur = 'utilisateur'
                    ) 
                    OR c.dateEnvoi > (
...
function rechercherTicket($bdd, $idTicket) {
    ...
    $requete = $bdd->prepare("SELECT t.*, u.nomUtilisateur, u.prenomUtilisateur, u.idEntreprise, tech.nomTechnicien, tech.prenomTechnicien, tech.role as roleTechnicien,
                         CASE WHEN t.description LIKE '%[Ticket créé par le technicien%' THEN true ELSE false END as creerParTechnicien
                         FROM ticket t 
                         LEFT JOIN utilisateur u ON t.idUtilisateur = u.idUtilisateur 
                         LEFT JOIN techniciens tech ON t.idTechnicien = tech.idTechnicien 
    ...
    if ($ticket) {
        ...
        // Si la session est directeur, on force l'affichage Directeur
        if (isset($_SESSION['user']['role']) && $_SESSION['user']['role'] === 'directeur') {
            $ticket['assignee'] = [
                'nom' => isset($_SESSION['user']['prenom']) ? $_SESSION['user']['prenom'] . ' ' . ($_SESSION['user']['nom'] ?? '') : 'Directeur',
                'role' => 'Directeur',
                'avatar' => strtoupper(substr($_SESSION['user']['prenom'] ?? 'D', 0, 1) . substr($_SESSION['user']['nom'] ?? 'R', 0, 1))
            ];
        } else if ($ticket['nomTechnicien'] || $ticket['prenomTechnicien']) {
            $ticket['assignee'] = [
                'nom' => $ticket['prenomTechnicien'] . ' ' . $ticket['nomTechnicien'],
                'role' => $ticket['roleTechnicien'] ?? '',
