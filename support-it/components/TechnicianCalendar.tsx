  ...
  useEffect(() => {
    ...
    if (userData) {
      try {
        ...
        idTechnicien = user.id || user.idTechnicien;
      } catch {}
    }
    if (!idTechnicien) {
      setAppointments({});
    ...
    fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/obtenirRdvTechnicien.php?idTechnicien=${idTechnicien}`, {
      credentials: 'include',
    })
      .then((res) => res.json())
        ...
        >
          ...
          >
            ...
            <dl className="text-sm text-slate-600 flex flex-col gap-2">
              ...
              <div>
                <span className="font-medium text-slate-700">Client :</span> {modal.appointment.client}
                {modal.appointment.telephoneClient && (
                  <span className="ml-2 text-slate-500">{modal.appointment.telephoneClient}</span>
                )}
              </div>
            ...
            <div className="flex gap-3 mt-6">
              ...
              <Button
                ...
                className="flex-1"
                onClick={() => {
                  alert(`Modifier le rendez-vous ${modal.appointment!.id} - À implémenter`);
                  closeModal();
                }}
              >
                Modifier
