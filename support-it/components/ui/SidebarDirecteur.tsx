...
import { Home, Ticket, ClipboardList, Wrench, Users, Building2, BarChart3, Archive, Calendar, SlidersHorizontal, Settings, Bell, User } from 'lucide-react';
import SidebarShell, { NavSection } from './SidebarShell';
import useDirecteurNotifications from '../hooks/useDirecteurNotifications';

export default function SidebarDirecteur({ user }: { user: { nom: string; prenom: string; role: string } | null }) {
  ...
  const sections: NavSection[] = [
    ...
    },
    {
      ...
      items: [
        ...
        { label: 'Configuration', href: '/directeur/configuration', icon: <SlidersHorizontal size={16} /> },
  ...
  return (
    <SidebarShell
      ...
      sections={sections}
      user={user ? { nom: user.nom, prenom: user.prenom, roleLabel: 'Directeur' } : null}
    />
  );
}
